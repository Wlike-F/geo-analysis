  ...
  <div class="files-container">
    ...
    <el-card shadow="never" class="mb-20 step-card">
      <el-steps :active="activeStep" align-center finish-status="success">
        ...
        <el-step title="第二步：阈值筛�? description="多文件或单文件条件过�? />
    ...
    <div style="display: flex; gap: 20px;" class="split-layout">
      ...
      <div v-if="tabs.length === 0" class="empty-state">
        <el-empty description="暂无测井数据，请在上方加载文�? />
      </div>
            ...
            <el-row :gutter="20" align="middle" style="width: 100%;">
                ...
                <el-row :gutter="20">
                  <el-col :xs="24" :sm="12" :md="12" style="margin-bottom: 12px;" v-for="col in item.columns" :key="col">
                    <div class="flex-center">
                      ...
                      <el-input v-model="item.filters[col].min" placeholder="最小�? clearable style="width: 120px;" />
                      <span class="separator" style="margin: 0 10px; color: #909399;">-</span>
                      <el-input v-model="item.filters[col].max" placeholder="最大�? clearable style="width: 120px;" />
                    </div>
                  </el-col>
                </el-row>
              </el-col>
              ...
              <el-col :span="8" style="display: flex; flex-direction: column; align-items: flex-end; justify-content: center; border-left: 1px solid #EBEEF5; padding-left: 20px;">
                <div style="margin-bottom: 12px; display: flex; gap: 10px;">
                  ...
                  <el-button type="info" icon="Refresh" @click="clearFilters(item)" plain>清除筛�?/el-button>
                ...
                <div class="count-tip" style="font-size: 13px; color: #606266;">
                  筛选总记录数: <strong class="text-primary" style="font-size: 16px;">{{ item.totalRows }}</strong> �?
                </div>
              </el-col>
            </el-row>
          </div>

          <!-- 第一版表格：前端截取�?300 行展示数�?(或使用过滤后�?displayData ) -->
    ...
    <el-card shadow="always" class="analysis-card" style="width: 320px; min-height: 500px; flex-shrink: 0;" v-if="tabs.length > 0">
      ...
      <el-form label-position="top" size="small">
        ...
        <el-form-item label="判断条件">
          <div style="display: flex; gap: 10px;">
            ...
            <el-input v-model="analysisParams.threshold" type="number" placeholder="阈�? />
          </div>
        </el-form-item>

        <el-form-item label="最短连续测点数 (去除噪点)">
      ...
      <div v-if="analysisResults.length > 0" style="margin-top: 25px; border-top: 1px dashed #dcdfe6; padding-top: 15px;">
        <div style="font-size: 14px; color: #606266; margin-bottom: 15px; display: flex; justify-content: space-between; align-items: center;">     
          <span>检测出 <strong style="color: #f56c6c;">{{ analysisResults.length }}</strong> 段异常地�?/span>
          ...
          <el-timeline>
            <el-timeline-item v-for="(seg, index) in analysisResults" :key="index" type="primary" :hollow="true">
              ...
              <div style="font-size: 12px; color: #e6a23c; margin-top: 4px;">   
                点数: {{ seg.pointCount }} | 均�? {{ seg.avgValue.toFixed(4) }} 
              </div>
            </el-timeline-item>
          </el-timeline>
        </div>
    ...
    <el-dialog v-model="fileDialogVisible" title="选择测井文件" width="70%" destroy-on-close>
      ...
      >
        ...
        <el-table-column prop="totalRows" label="总行�? width="120"></el-table-column>
...
import { getFileList, getPageData, exportFilteredExcel, getFilePage } from '@/api/file'
...
const downloadAnalysisExcel = () => {
  ...
  ElMessage.success('结果成果表导出完成！系统已利�?Excel/CSV 格式进行保存�?)
}


const extractContinuousSegments = async () => {
  if (!activeTabName.value) {
    ElMessage.warning('请先选择一个数据页签进行分�?)
    return
  }

  const currentTab = tabs.value.find(t => t.name === activeTabName.value)
  if (!currentTab) {
    ElMessage.warning('当前页签无数�?)
    return
  }
  ...
  if (isNaN(threshold)) {
    ElMessage.warning('请输入有效的阈�?)
    return
  }

  ElMessage.info('正在极速拉取后端全量日志序列，请稍�?..');
  let targetData = [];
  try {
  ...
  for (let i = 0; i < targetData.length; i++) {
    ...
    const depthCol = currentTab.columns.find(c => ['DEPTH','深度','TVD'].includes(c.toUpperCase()) || c.includes('�?)) || currentTab.columns[0];
  ...
  if (!analysisResultsEmpty.value) {
    ElMessage.success(`分析完成，共提取�?${segments.length} 段连续地层异常`)
  }
}
...
const applyFilters = (tabObj) => {
  ...
  fetchPageData(tabObj);
  ElMessage.success('已应用新的阈值筛选条件，正在拉取大数�?..');
}
...
const exportToExcel = async (tabObj) => {
  ...
  try {
    ElMessage.info('正在请求后端生成大数据过滤归�?Excel，这可能需要几十秒，请稍�?..')
    const filters = buildFiltersParam(tabObj)
    ...
    link.style.display = 'none'
    link.href = url
    link.setAttribute('download', tabObj.title + '_大数据归档报�?xlsx')
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
    window.URL.revokeObjectURL(url)
...
const handleExportCommand = async (command) => {
  ElMessage.warning('新版海量数据框架下，受堆内存限制，请使用单一文件的[生成 Excel]功能�?)
}
</script>
