...
const fallbackData = [
  {
    ...
    icon: 'Reading',
    category: 'quickstart',
    summary: '首次登录后，建议先完成数据上传并在看板确认解析结果。',
    steps: [
      '进入“数据源管理”，选择目标数据源。',
      '上传 TXT/CSV 文件并确认字段映射。',
      '进入“测井数据看板”检查数据是否加载成功。'
    ],
    note: '若加载时间较长，请先确认文件体积与网络状态。',
    content: '新手入门 上传 数据源 看板',
  ...
  },
  {
    ...
    icon: 'DocumentCopy',
    category: 'feature',
    summary: '导出前建议先缩小筛选范围，避免超大文件导出失败。',
    steps: [
      '在看板完成筛选并确认结果数量。',
      '点击导出按钮，按需求选择 Excel 或 CSV。',
      '下载完成后先抽样核对关键字段。'
    ],
    note: 'Excel 单表存在行数上限，超大数据建议使用 CSV。',
    content: '导出 报表 excel csv',
    links: [{ label: '去测井数据看板', path: '/files' }]
  },
  {
    id: 'local-3',
    title: '常见问题：导出失败与解析异常',
    icon: 'Bell',
    category: 'faq',
    summary: '出现报错时优先检查数据量、字段映射、筛选条件与文件格式。',
    steps: [
      '确认导出数据量是否超过目标格式限制。',
      '核对字段映射和筛选条件是否有效。',
      '必要时拆分导出或切换为 CSV 后重试。'
    ],
    note: '若多次失败，请保留报错信息并联系管理员排查。',
    content: '常见问题 报错 异常 失败',
